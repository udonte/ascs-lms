...
export async function ensureUserProfile(
  ...
  // Only create the profile row for brand-new users.
  // Never overwrite an existing profile — the user may have updated their
  // display name via the Settings page, and auth metadata (user_metadata.full_name)
  // never reflects those changes, so writing it back here would reset the name.
  if (!existing) {
